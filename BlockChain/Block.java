...
class Node extends Thread {
    public void run() {
        ...
        System.out.println("Node: "+Thread.currentThread().getName()+" started.");
        Block.addBlocks(Block.blockchain, Block.prefix,
        Block.prefixString,0,2,25.3f);
...
public static void addBlocks(List<Block> blockchain,int prefix, String prefixString, int senderId, int recieverId, float amount) {
    String prev = "0";
    if(blockchain.size() != 0){
        prev = blockchain.get(blockchain.size() - 1).getHash();
    }
    Block newBlock = new Block("The is a New Block.", prev, new
    Date().getTime(), senderId, recieverId, amount);
    newBlock.mineBlock(prefix);
    synchronized(blockchain) {
        if(Block.flag==0) {
        blockchain.add(newBlock);
        Block.flag=1;
        System.out.println("Block added by node:
...
public static void validate(List<Block> blockchain,int prefix, String prefixString) {
    ...
    if(flag) {
        System.out.println("Block verified by node:
        "+Thread.currentThread().getName());
        }
    }
...
public static void main(String[] args) {
    ...
    flag=0;
    addBlocks(blockchain, prefix, prefixString,-1,0,200);
    13
    flag=0;
